...
- generate_host_vars_structured.py を中心とした責務分割を, 図と本文の両方で追えるようにする。
...
| スクリプト | 主な責務 |
| --- | --- |
| generate_host_vars_structured.py | network_topology.yaml から host_vars_structured.yaml を生成する主オーケストレータ |
| generate_hostvars_matrix.py | host_vars_structured.yaml から host_vars_scalars_matrix.csv を生成する |
| validate_hostvars_matrix.py | CSV の整合性を検証する |
| generate_host_vars_files.py | host_vars_structured.yaml から host_vars.gen 配下のホスト変数ファイル群を生成する |
| generate_terraform_tfvars.py | network_topology.yaml から terraform.tfvars を生成する |
...
| 入力 | field_metadata.yaml | CSV と host_vars コメント生成のメタデータ |
...
通常系では, network_topology.yaml を中間構造へ変換し, そこから CSV と host_vars 群を派生させる。Terraform 系はこの流れとは独立に, 特定ロールを持つノード群から terraform.tfvars を生成する。

### 4.1 通常系フローの概要

通常系の主経路は次のとおりである。

1. generate_host_vars_structured.py が network_topology.yaml と convert-rule-config.yaml を読み込む。
2. lib 配下のモジュールが, ロール解決, クラスタ正規化, スカラー導出, ルーティング情報生成を分担する。
3. host_vars_structured.yaml を生成する。
4. generate_hostvars_matrix.py が CSV を生成する。
5. validate_hostvars_matrix.py が CSV の整合性を検証する。
6. generate_host_vars_files.py が host_vars.gen 配下の最終成果物を出力する。

### 4.2 Terraform 生成フローの概要

Terraform 系の主経路は次のとおりである。

1. generate_terraform_tfvars.py が network_topology.yaml を読み込む。
...
中間生成物の中核は host_vars_structured.yaml である。このファイルは, 以後の CSV 生成と host_vars 群生成の双方の入力になる。
...
| CLI | 主入力 | 主出力 | 備考 |
| --- | --- | --- | --- |
| generate_host_vars_structured.py | network_topology.yaml, convert-rule-config.yaml | host_vars_structured.yaml | 通常系の起点 |
| generate_hostvars_matrix.py | host_vars_structured.yaml, field_metadata.yaml | host_vars_scalars_matrix.csv | 表形式での確認用途 |
| validate_hostvars_matrix.py | host_vars_scalars_matrix.csv, field_metadata.yaml, host_vars_structured.yaml | 検証結果 | エラー時は終了コード非 0 |
| generate_host_vars_files.py | host_vars_structured.yaml, field_metadata.yaml | host_vars.gen 配下 | コメント付き host_vars を生成 |
| generate_terraform_tfvars.py | network_topology.yaml | terraform.tfvars | Terraform 系は独立分岐 |
...
component "generate_host_vars_structured.py" as gen_structured
artifact "host_vars_structured.yaml" as structured

component "generate_hostvars_matrix.py" as gen_matrix
artifact "host_vars_scalars_matrix.csv" as matrix

component "validate_hostvars_matrix.py" as validate_matrix

component "generate_host_vars_files.py" as gen_files
folder "host_vars.gen/" as hostvars_dir

component "generate_terraform_tfvars.py" as gen_tfvars
...
通常系の中心は host_vars_structured.yaml であり, 以降の CSV 生成と host_vars 生成はこの中間成果物を共有する。Terraform 系は同じ network_topology.yaml を参照するが, 変換経路と出力物は分離されている。
...
package "CLI scripts" {
  class "generate_host_vars_structured.py" as GenerateStructured
  class "generate_hostvars_matrix.py" as GenerateMatrix
  class "validate_hostvars_matrix.py" as ValidateMatrix
  class "generate_host_vars_files.py" as GenerateFiles
  class "generate_terraform_tfvars.py" as GenerateTfvars
}
...
generate_host_vars_structured.py は, CLI 群の中で最も多くの lib モジュールへ依存する。これは, このスクリプトが変換規則の読込, 実行時コンテキスト準備, ノード単位組み立て, YAML 出力の順を制御する主オーケストレータだからである。
...
このフローでは, generate_host_vars_structured.py が各サブモジュールの呼び出し順を制御する。ノード単位の繰り返し部分では, 初期化, NIC 処理, ルーティング出力, サービススカラー適用が順番に実行される。
...
hostvars_node_pipeline.py は, host_entry の初期化, netif_list 生成, FRR 関連出力, サービススカラー適用を段階的に分離している。これにより, generate_host_vars_structured.py 側はオーケストレーションに専念できる。
...
| lib の責務分割変更 | パッケージ図 |
| 抽象インターフェース変更 | クラス図 |
| generate_host_vars_structured.py の呼び出し順変更 | シーケンス図 9.1 |
| generate_terraform_tfvars.py の処理段数変更 | シーケンス図 9.2 |
...
- src/prototype/generate_host_vars_structured.py
- src/prototype/generate_hostvars_matrix.py
- src/prototype/validate_hostvars_matrix.py
- src/prototype/generate_host_vars_files.py
- src/prototype/generate_terraform_tfvars.py
