...
4. [変換ルール設定リファレンスマニュアル](convert-rule-config-reference.md): `convert-rule-config.yaml` の仕様確認
5. [フィールドメタデータリファレンスマニュアル](field-metadata-reference.md): `field_metadata.yaml` の仕様確認
...
| ファイル | 説明 |
|---|---|
| [スキーマファイルリファレンスマニュアル](schema-files-reference.md) | `network_topology.schema.yaml`, `host_vars_structured.schema.yaml`, `field_metadata.schema.yaml`, `type_schema.yaml` の仕様 |
| [変換ルール設定リファレンスマニュアル](convert-rule-config-reference.md) | `convert-rule-config.yaml` の `service_settings`, `service_transform`, `network_role` セクションの仕様 |
| [フィールドメタデータリファレンスマニュアル](field-metadata-reference.md) | `field_metadata.yaml` の各エントリのキーと制約の仕様 |
...
| [generate-host-vars-structured.md](generate-host-vars-structured.md) | `generate_host_vars_structured.py` の CLI マニュアル | ansible-linux-setup 利用者 |
| [generate-hostvars-matrix.md](generate-hostvars-matrix.md) | `generate_hostvars_matrix.py` の CLI マニュアル | ansible-linux-setup 利用者 |
| [generate-host-vars-files.md](generate-host-vars-files.md) | `generate_host_vars_files.py` の CLI マニュアル | ansible-linux-setup 利用者 |
| [validate-hostvars-matrix.md](validate-hostvars-matrix.md) | `validate_hostvars_matrix.py` の CLI マニュアル | ansible-linux-setup 利用者 |
| [generate-terraform-tfvars.md](generate-terraform-tfvars.md) | `generate_terraform_tfvars.py` の CLI マニュアル | ansible-linux-setup 利用者 |
| [generate-network-topology-design-sheet.md](generate-network-topology-design-sheet.md) | `generate_network_topology_design_sheet.py` の CLI マニュアル | ansible-linux-setup 利用者 |
